...
**Selection is handled in `onPointerDownCapture`** (capture phase, before moveable's gesto): for an unselected element it calls `flushSync(() => select([id]))` so moveable re-binds its gesto in the same event tick and the still-propagating pointerdown is naturally caught — enabling direct click-drag of unselected elements. Elements marked `data-no-drag` (container header buttons) are skipped. This capture-phase + flushSync approach is used **instead of** moveable's `dragStart()` API (programmatic `dragStart` leaves gesto unable to release → stuck drag). **Empty-canvas pointerdown is NOT handled here** - it only activates the template then returns; box-select / click-to-clear is owned by `MarqueeSelect` (see next paragraph). (Previously this called `clearSelection()` on empty canvas; it was removed because it wiped selection before a Shift-additive marquee could accumulate.)

**Marquee (drag-to-select) box selection** is `MarqueeSelect.jsx` (`react-selecto`, the moveable author's companion lib). It renders a `<Selecto>` inside `InfiniteViewer` as a sibling of `MoveableLayer`. Pointer arbitration is a single gate in Selecto's `dragCondition`: only an empty-canvas left-drag starts a marquee; it bails when `canDrag` (space-pan / pan-toggle), `dndActive` (palette drag), or the target is a moveable handle / element (`[data-id]`) / chrome (`[data-no-drag]`, `[data-zoom-bar]`, `.ruler-area`). Coordinate matrix: `rootContainer` = un-transformed `canvasWrapRef`, `container` = zoom-transformed `gridRef`, so the rect aligns at any zoom. `onSelectEnd` maps hit DOM (`data-id`) → ids, filters to the active template (multi-board safety), and calls `select(ids)` (`selectAtom` auto-expands groups) → `selectedIdsAtom` → `MoveableLayer` picks up targets with **zero moveable-side change**. Shift-drag unions with current selection; plain drag replaces; empty click → `select([])` clears. Space-held = pan, plain empty drag = marquee (Figma model). `MarqueeSelect` is `memo`'d and reads `elementsAtom`/`activeTemplateIdAtom` lazily via `useStore.get` in `onSelectEnd` (not subscribed), so it doesn't re-render on element mutations.
